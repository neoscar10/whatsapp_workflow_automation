<div x-data="{ showLeftSidebar: true, showRightSidebar: true }" class="flex flex-1 w-full relative overflow-hidden bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 antialiased min-h-[500px]">
    ...
    <script>
        ...
        let companyId = @js(auth()->user()->company_id);
        ...
        function subscribeToConversation(id) {
            ...
            window.Echo.private(`company.${companyId}.conversation.${id}`)
                .listen('.message.received', (e) => {
                    console.log('Message received via Echo:', e);
                    $wire.dispatch('realtime-message-received', { payload: e });
                });
        }

        function unsubscribeFromConversation(id) {
            if (!id) return;
            console.log('Leaving conversation:', id);
            window.Echo.leave(`company.${companyId}.conversation.${id}`);
        }

        // Global company chats listener (sidebar updates)
        window.Echo.private(`company.${companyId}.chats`)
            .listen('.conversation.updated', (e) => {
                console.log('Conversation updated via Echo:', e);
                $wire.dispatch('realtime-conversation-updated', { payload: e });
            });
        ...
        $wire.on('conversation-selected', (data) => {
            const newId = data[0].conversation_id;

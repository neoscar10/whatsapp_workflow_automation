                ...
                <section class="flex flex-1 flex-col border-r border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
                    ...
                    >
                        @foreach($messages as $message)
                            ...
                            @elseif(in_array($message['message_type'], ['image', 'video', 'audio', 'document', 'sticker']))
                                <div class="flex max-w-[80%] flex-col {{ $message['direction'] === 'outbound' ? 'self-end items-end' : 'items-start' }}">
                                    <div class="rounded-2xl {{ $message['direction'] === 'outbound' ? 'rounded-tr-none' : 'rounded-tl-none' }} border border-slate-100 bg-white p-1.5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                                        ...
                                        @elseif($message['message_type'] === 'sticker')
                                            <div class="h-36 w-36 overflow-hidden rounded-2xl bg-transparent group relative flex items-center justify-center p-1">
                                                @if(!empty($message['resolved_media_url']))
                                                    <img 
                                                        src="{{ $message['resolved_media_url'] }}" 
                                                        alt="Sticker" 
                                                        class="h-full w-full object-contain drop-shadow-md transition-transform group-hover:scale-105"
                                                        onerror="this.onerror=null; this.src='https://placehold.co/200x200/1e293b/64748b?text=Sticker'; this.parentElement.classList.add('opacity-50');"
                                                    >
                                                    <a href="{{ $message['resolved_media_url'] }}" target="_blank" class="absolute inset-0 flex items-center justify-center bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity rounded-2xl">
                                                        <span class="material-symbols-outlined text-white">open_in_new</span>
                                                    </a>
                                                @else
                                                    <div class="flex h-full w-full items-center justify-center bg-slate-100/50 dark:bg-slate-800/50 rounded-2xl">
                                                        <div class="text-center p-2">
                                                            <span class="material-symbols-outlined text-slate-400 text-3xl">sentiment_satisfied</span>
                                                            <p class="text-[9px] text-slate-500 font-bold uppercase mt-1">Sticker</p>
                                                        </div>
                                                    </div>
                                                @endif
                                            </div>
                                        ...
                                        @if(!empty($message['body']) && $message['message_type'] !== 'sticker' && strtolower(trim($message['body'])) !== 'sticker')

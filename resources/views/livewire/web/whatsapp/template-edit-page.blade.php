<div class="mx-auto w-full max-w-[1400px] space-y-8 p-6 md:p-10">
    {{-- Header Section --}}
    <div class="flex items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-6">
        <div class="flex items-center gap-4">
            <a href="{{ route('whatsapp.templates.show', $template->id) }}" class="group flex h-10 w-10 items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors cursor-pointer">
            ...
            <div>
                <h1 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-3">
                    ...
                    <span class="inline-flex items-center rounded-md bg-slate-100 dark:bg-slate-800 px-2 py-1 text-sm font-medium text-slate-600 dark:text-slate-300 ring-1 ring-inset ring-slate-500/10 capitalize">{{ $template->status }}</span>
                </h1>
                <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">Modify your template content and resubmit to Meta.</p>
            </div>
        </div>
        
        <div>
            <a href="{{ route('whatsapp.templates.show', $template->id) }}" class="inline-flex items-center gap-2 rounded-xl bg-slate-100 dark:bg-slate-800 px-4 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 cursor-pointer transition-colors">
                Cancel Edit
            </a>
        </div>
    </div>
    ...
    @if (session()->has('warning'))
        <div class="rounded-xl bg-amber-50 dark:bg-amber-900/20 p-4 border border-amber-200 dark:border-amber-900/30">
            <div class="flex">
    ...
    @if ($errors->has('api'))
        <div class="rounded-xl bg-red-50 dark:bg-red-900/20 p-4 border border-red-200 dark:border-red-900/30">
            <div class="flex">
                ...
                <div class="ml-3">
                    <p class="text-sm font-medium text-red-800 dark:text-red-300">{{ $errors->first('api') }}</p>
                </div>
            </div>
        </div>
    @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                ...
                <div class="p-6 space-y-8">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <p class="text-sm text-slate-500 dark:text-slate-400">Name and language cannot be changed once a template is created on Meta.</p>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-sm font-semibold text-slate-900 dark:text-white">Template Name</label>
                                <div class="mt-2">
                                    <input type="text" value="{{ $name }}" disabled class="block w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/60 px-4 py-2.5 text-slate-500 dark:text-slate-400 shadow-sm sm:text-sm">
                                </div>
                            </div>

                            <div>
                                <label class="block text-sm font-semibold text-slate-900 dark:text-white">Language</label>
                                <div class="mt-2">
                                     <input type="text" value="{{ strtoupper($language) }}" disabled class="block w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/60 px-4 py-2.5 text-slate-500 dark:text-slate-400 shadow-sm sm:text-sm">
                                </div>
                            </div>
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <p class="text-sm text-slate-500 dark:text-slate-400">Update the structure of your message. Use @{{1}} to insert variables.</p>
                        </div>
                    ...
                    <div class="space-y-6">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="relative" x-data="{ open: false }">
                                <button @click="open = !open" @click.away="open = false" type="button" class="inline-flex items-center gap-2 rounded-xl bg-white dark:bg-slate-800 px-4 py-2 text-sm font-semibold text-slate-700 dark:text-slate-200 shadow-sm ring-1 ring-inset ring-slate-300 dark:ring-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer transition-colors">
                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
                                    Add Button
                                </button>
                                
                                <div x-show="open" style="display: none;" class="absolute right-0 z-10 mt-2 w-48 rounded-xl bg-white dark:bg-slate-800 p-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none dark:border dark:border-slate-700">
                                    <button wire:click="addButton('quick_reply')" @click="open = false" class="block w-full text-left rounded-lg px-4 py-2 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-white cursor-pointer">Quick Reply</button>
                                    <button wire:click="addButton('url')" @click="open = false" class="block w-full text-left rounded-lg px-4 py-2 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-white cursor-pointer">Visit Website</button>
                                    <button wire:click="addButton('phone_number')" @click="open = false" class="block w-full text-left rounded-lg px-4 py-2 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-white cursor-pointer">Call Phone</button>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="space-y-4">
                                @foreach($buttons as $index => $button)
                                    <div class="bg-white dark:bg-slate-900 border rounded-xl border-slate-200 dark:border-slate-800 p-4 shadow-sm flex items-start gap-4" wire:key="button-{{ $index }}">
                                        <div class="mt-2 p-2 bg-slate-100 dark:bg-slate-800 rounded-lg shrink-0">
                                        ...
                                        <button wire:click="removeButton({{ $index }})" type="button" class="shrink-0 p-2 text-slate-400 hover:text-red-500 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer">
                        ...
                        @else
                            <div class="rounded-xl border border-dashed border-slate-300 dark:border-slate-700 p-8 text-center bg-slate-50 dark:bg-slate-800/30">
                                <span class="block text-sm text-slate-500 dark:text-slate-400">No buttons added.</span>
                            </div>
                        @endif
                    </div>
                </div>

                {{-- Action / Footer Bar with Submit Button --}}
                <div class="bg-slate-50 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-800 px-6 py-4 flex items-center justify-end gap-3">
                    <a href="{{ route('whatsapp.templates.show', $template->id) }}" class="inline-flex items-center justify-center rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-4 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-300 shadow-sm hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer transition-all">
                        Cancel
                    </a>
        ...
        <div class="lg:col-span-1 border-4 border-slate-900/10 dark:border-white/10 rounded-[2.5rem] bg-slate-50 dark:bg-slate-900 relative h-auto shadow-2xl p-4 overflow-hidden sticky top-8">
            <div class="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-6 bg-slate-900/10 dark:bg-white/10 rounded-b-2xl"></div>
            
            <div class="bg-[#efeae2] dark:bg-slate-950 rounded-3xl h-full shadow-inner overflow-hidden flex flex-col pt-12">
                ...
                <div class="flex-1 p-4 overflow-y-auto space-y-4 pb-20 no-scrollbar">
                    ...
                    <div class="bg-[#d9fdd3] dark:bg-emerald-950/60 dark:text-emerald-100 p-3 rounded-lg rounded-tr-none shadow-sm max-w-[85%] ml-auto text-sm text-slate-800">
                        Trigger Edit Simulation
                        <div class="text-[10px] text-slate-500 dark:text-slate-400 mt-1 text-right">Just now</div>
                    </div>

                    <div class="bg-white dark:bg-slate-900 p-2 rounded-lg rounded-tl-none shadow-sm max-w-[90%] space-y-2 border dark:border-slate-800">
                        ...
                        <div class="px-1 text-sm text-slate-800 dark:text-slate-200 leading-relaxed whitespace-pre-wrap">
                            @if(empty($bodyText))
                        ...
                        @if($footerText)
                            <div class="px-1 text-[11px] text-slate-500 dark:text-slate-400 mt-1">{{ $footerText }}</div>
                        @endif

                        @if(count($buttons) > 0)
                            <div class="space-y-1.5 pt-2 mt-2 border-t border-slate-100 dark:border-slate-800">
                                @foreach($buttons as $btn)
                                    <div class="w-full text-center text-[#00a884] dark:text-emerald-400 font-semibold text-sm py-2">
                                        @if($btn['type'] === 'url')

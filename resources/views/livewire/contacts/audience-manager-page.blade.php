<div class="flex flex-col flex-1 min-h-full bg-slate-50 dark:bg-slate-900/50">
                                ...
                                <tr class="group hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-all">
                                    ...
                                    <td class="px-8 py-5 text-right">
                                        <div class="flex items-center justify-end gap-1">
                                            ...
                                            <button wire:click="openDeleteGroupModal({{ $item->id }})" class="p-2 text-slate-400 hover:text-rose-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all" title="Delete Group">
                            ...
                            <div class="space-y-2">
                                @forelse($currentMembers as $member)
                                    <div class="flex items-center gap-3 p-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 group/item">
                                        ...
                                        <button wire:click="openRemoveMemberModal({{ $member->id }})" class="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-lg transition-all opacity-0 group-hover/item:opacity-100" title="Remove Member">
    ...
    @if($showDeleteGroupModal)
        <div class="fixed inset-0 z-[130] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" wire:keydown.escape.window="closeDeleteGroupModal">
            <div class="bg-white dark:bg-slate-900 w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col relative z-30 border border-slate-100 dark:border-slate-800 animate-in fade-in zoom-in duration-200">
                <div class="p-8 text-center flex flex-col items-center">
                    <div class="size-16 rounded-3xl bg-rose-50 dark:bg-rose-950/50 text-rose-500 flex items-center justify-center mb-6 shadow-inner">
                        <span class="material-symbols-outlined text-[36px]">delete_forever</span>
                    </div>
                    <h2 class="text-xl font-black text-slate-900 dark:text-white tracking-tight">Delete Audience Group?</h2>
                    <p class="text-xs text-slate-500 dark:text-slate-400 font-medium mt-3 leading-relaxed">
                        Are you sure you want to delete <strong class="text-slate-900 dark:text-white font-bold">{{ $deletingGroupName }}</strong>? Contacts in this group will not be deleted from your system.
                    </p>
                </div>
                <div class="px-8 py-5 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-3">
                    <button wire:click="closeDeleteGroupModal" class="px-6 py-2.5 text-xs font-black uppercase tracking-widest text-slate-400 hover:text-slate-600 transition-all">
                        Cancel
                    </button>
                    <button wire:click="confirmDeleteGroup" class="px-8 py-2.5 text-xs font-black uppercase tracking-widest text-white bg-rose-600 hover:bg-rose-700 rounded-2xl shadow-lg shadow-rose-600/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center gap-2">
    ...
    {{-- Remove Member Custom Modal --}}
    @if($showRemoveMemberModal)
        <div class="fixed inset-0 z-[130] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" wire:keydown.escape.window="closeRemoveMemberModal">
            <div class="bg-white dark:bg-slate-900 w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col relative z-30 border border-slate-100 dark:border-slate-800 animate-in fade-in zoom-in duration-200">
                <div class="p-8 text-center flex flex-col items-center">
                    <div class="size-16 rounded-3xl bg-amber-50 dark:bg-amber-950/50 text-amber-500 flex items-center justify-center mb-6 shadow-inner">
                        <span class="material-symbols-outlined text-[36px]">person_remove</span>
                    </div>
                    <h2 class="text-xl font-black text-slate-900 dark:text-white tracking-tight">Remove Member?</h2>
                    <p class="text-xs text-slate-500 dark:text-slate-400 font-medium mt-3 leading-relaxed">
                        Are you sure you want to remove <strong class="text-slate-900 dark:text-white font-bold">{{ $removingContactName }}</strong> from this audience group?
                    </p>
                </div>
                <div class="px-8 py-5 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-3">
                    <button wire:click="closeRemoveMemberModal" class="px-6 py-2.5 text-xs font-black uppercase tracking-widest text-slate-400 hover:text-slate-600 transition-all">
                        Cancel
                    </button>
                    <button wire:click="confirmRemoveMember" class="px-8 py-2.5 text-xs font-black uppercase tracking-widest text-white bg-amber-600 hover:bg-amber-700 rounded-2xl shadow-lg shadow-amber-600/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center gap-2">

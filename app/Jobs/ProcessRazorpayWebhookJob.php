...
class ProcessRazorpayWebhookJob implements ShouldQueue
{
    ...
    public function handle(PaymentService $paymentService): void
    {
        ...
        Log::info("ProcessRazorpayWebhookJob started processing", [
            'event' => $event,
            'job_id' => $this->job?->getJobId()
        ]);

        try {
            $paymentService->handleWebhookEvent($event, $this->payload);
        } catch (\Exception $e) {
            Log::error("ProcessRazorpayWebhookJob failed during handle", [
                'event' => $event,
                'error' => $e->getMessage(),
                'exception' => $e
            ]);

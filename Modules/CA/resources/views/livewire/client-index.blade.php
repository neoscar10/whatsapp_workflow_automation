<div class="p-6 lg:p-8 w-full max-w-7xl mx-auto font-['Inter']">
                    ...
                    @forelse($clients as $client)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                            <td class="p-4 whitespace-nowrap">
                                <a href="{{ $client->status === 'draft' ? route('ca.clients.onboard', ['draft_id' => $client->id]) : route('ca.clients.show', $client->id) }}" class="flex items-center gap-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 p-2 -ml-2 rounded-lg transition-colors group">
                                    <div class="size-10 rounded-xl bg-primary/10 dark:bg-primary/20 flex items-center justify-center text-primary dark:text-blue-400 font-bold overflow-hidden group-hover:scale-105 transition-transform">
                                        {{ substr($client->client_name, 0, 1) }}
                                    </div>
                                    <div>
                                        <p class="text-sm font-bold text-slate-900 dark:text-white group-hover:text-primary dark:group-hover:text-blue-400 transition-colors">{{ $client->client_name }}</p>
                            ...
                            <td class="p-4 text-right">
                                <div x-data="{ open: false }" class="relative inline-block text-left">
                                    <button @click="open = !open" @click.away="open = false" class="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 rounded-lg transition-colors">
                                        <span class="material-symbols-outlined text-[20px]">more_vert</span>
                                    </button>
                                    <div x-show="open" x-transition style="display: none;" class="absolute right-4 z-50 w-44 mt-1 bg-white dark:bg-slate-800 border border-[#c2c6d8]/50 dark:border-slate-700 rounded-xl shadow-lg focus:outline-none overflow-hidden">
                                        <div class="py-1">
                                            @if($client->status === 'draft')
                                                <a href="{{ route('ca.clients.onboard', ['draft_id' => $client->id]) }}" class="flex items-center gap-2 px-4 py-2 text-[13px] font-medium text-[#424656] dark:text-slate-300 hover:bg-[#f6f3f2] dark:hover:bg-slate-700/50 hover:text-orange-500 transition-colors whitespace-nowrap">
                                                    <span class="material-symbols-outlined text-[18px]">edit_document</span> Resume
                                                </a>
                                            @else
                                                <a href="{{ route('ca.clients.show', $client->id) }}" class="flex items-center gap-2 px-4 py-2 text-[13px] font-medium text-[#424656] dark:text-slate-300 hover:bg-[#f6f3f2] dark:hover:bg-slate-700/50 hover:text-primary transition-colors whitespace-nowrap">
                                                    <span class="material-symbols-outlined text-[18px]">visibility</span> View Profile
                                                </a>
                                            @endif
                                            
                                            <a href="{{ route('ca.clients.onboard', ['draft_id' => $client->id]) }}" class="flex items-center gap-2 px-4 py-2 text-[13px] font-medium text-[#424656] dark:text-slate-300 hover:bg-[#f6f3f2] dark:hover:bg-slate-700/50 hover:text-blue-500 transition-colors whitespace-nowrap">
                                                <span class="material-symbols-outlined text-[18px]">update</span> Update Client
                                            </a>
                                            
                                            <button wire:click="confirmDelete({{ $client->id }})" class="w-full text-left flex items-center gap-2 px-4 py-2 text-[13px] font-medium text-[#424656] dark:text-slate-300 hover:bg-[#ffdad6]/30 dark:hover:bg-red-900/20 hover:text-[#ba1a1a] transition-colors whitespace-nowrap">
                                                <span class="material-symbols-outlined text-[18px]">delete</span> Delete Client
                                            </button>
                                        </div>
                                    </div>
                                </div>
    ...
    @if($showDeleteModal)
        <div class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fade-in">
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-md border border-[#c2c6d8]/30 dark:border-slate-700 overflow-hidden transform scale-100 transition-all">
                <div class="px-6 py-5 border-b border-[#c2c6d8]/30 dark:border-slate-700 flex items-center justify-between">
                    <h3 class="text-lg font-bold text-[#ba1a1a] dark:text-red-500 flex items-center gap-2">
                        <span class="material-symbols-outlined">warning</span> Delete Client
                    </h3>
                    <button wire:click="cancelDelete" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
                        <span class="material-symbols-outlined text-[20px]">close</span>
                    </button>
                </div>
                
                <div class="p-6">
                    <p class="text-[14px] text-[#424656] dark:text-slate-300">
                        Are you sure you want to delete this client? This action will archive their record. You will still be able to register another client with the same email or phone number if needed.
                    </p>
                </div>
                
                <div class="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-t border-[#c2c6d8]/30 dark:border-slate-700 flex justify-end gap-3">
                    <button type="button" wire:click="cancelDelete" class="px-5 py-2.5 text-sm font-semibold text-[#424656] dark:text-slate-300 bg-white dark:bg-slate-800 border border-[#c2c6d8] dark:border-slate-600 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                        Cancel
                    </button>
                    <button type="button" wire:click="deleteClient" class="px-5 py-2.5 text-sm font-semibold text-white bg-[#ba1a1a] border border-[#ba1a1a] rounded-xl hover:bg-[#93000a] transition-colors shadow-sm flex items-center gap-2">
                        Delete Client
                    </button>
                </div>
            </div>

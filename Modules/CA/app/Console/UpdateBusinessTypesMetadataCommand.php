...
namespace Modules\CA\Console;

use Illuminate\Console\Command;
use Modules\CA\Models\CABusinessType;

class UpdateBusinessTypesMetadataCommand extends Command
{
    protected $signature = 'ca:update-business-metadata';
    protected $description = 'Updates CA business types with metadata from old blade array';
    ...
    public function handle()
    {
        ...
        foreach ($businessDetails as $name => $data) {
            $type = CABusinessType::where('name', $name)->first();
            if ($type) {
                $type->update([
                    'icon' => $data['icon'],
        ...
        $this->info("Done updating business types.");
    }
}
